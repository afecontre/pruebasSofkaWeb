...
public class AddItemInterestApprove implements Task {

    public AddItemInterestApprove() {
        //Nothing
    }

    public static AddItemInterestApprove withTheFollowingField() {
        return instrumented(AddItemInterestApprove.class);
    }
        ...
        for (String loanNumber : loanNumbers) {
            ...
            actor.should(seeThat(ObtainText.element(LBL_TOTAL_REQUESTING),containsString(total_requesting)).orComplainWith(NotFoundText.class,NotFoundText.THE_VALUE_IS_NOT_EXPECT));
                ...
                actor.should(
                        ...
                        seeThat(ObtainText.element(LBL_TOTAL_APPROVED),containsString(REQUESTING_INTEREST_RESERVE)).orComplainWith(NotFoundText.class,NotFoundText.THE_VALUE_IS_NOT_EXPECT),
                        seeThat(ObtainText.element(LBL_TOTAL_RETAINAGE),containsString(RETAINAGE_INTEREST_RESERVE)).orComplainWith(NotFoundText.class,NotFoundText.THE_VALUE_IS_NOT_EXPECT),
                        seeThat(ObtainText.element(LBL_TOTAL_APPROVED_LESS_RETAINAGE),containsString(OperationsMath.restarNumeros(REQUESTING_INTEREST_RESERVE,interest))).orComplainWith(NotFoundText.class,NotFoundText.THE_VALUE_IS_NOT_EXPECT));
