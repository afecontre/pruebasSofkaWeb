...
public class AddItemDepositsConfirmed implements Task {

    public AddItemDepositsConfirmed() {
       //Nothing
    }

    public static AddItemDepositsConfirmed withTheFollowingField() {
        return instrumented(AddItemDepositsConfirmed.class);
    }
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        for (String loanNumber : loanNumbers) {
            ...
            actor.should(
                    seeThat(ObtainText.element(LBL_TOTAL_REQUESTING),containsString(total_requesting)).orComplainWith(NotFoundText.class,NotFoundText.THE_VALUE_IS_NOT_EXPECT));

            actor.attemptsTo(
                    WaitUntil.the(BTN_DEPOSIT_ITEM_ON, isVisible()).forNoMoreThan(2).seconds(),
                    WaitUntil.the(BTN_APPROVED_ITEM_CONTINGENCY_OFF, isEnabled()).forNoMoreThan(2).seconds(),
                    WaitUntil.the(BTN_APPROVED_ITEM_INTEREST_RESERVE_OFF, isEnabled()).forNoMoreThan(2).seconds()
            );
            ...
            actor.should(
                    ...
                    seeThat(ObtainText.element(LBL_TOTAL_APPROVED),containsString(TOTAL_ZERO)).orComplainWith(NotFoundText.class,NotFoundText.THE_VALUE_IS_NOT_EXPECT),
                    seeThat(ObtainText.element(LBL_TOTAL_RETAINAGE),containsString(TOTAL_ZERO)).orComplainWith(NotFoundText.class,NotFoundText.THE_VALUE_IS_NOT_EXPECT),
                    seeThat(ObtainText.element(LBL_TOTAL_APPROVED_LESS_RETAINAGE),containsString(TOTAL_ZERO)).orComplainWith(NotFoundText.class,NotFoundText.THE_VALUE_IS_NOT_EXPECT));
